...
class TenantFileSystemStorage(FileSystemStorage):
    ...
    def relative_media_root(self):
        try:
            return os.path.join(
                settings.MEDIA_ROOT, settings.MULTITENANT_RELATIVE_MEDIA_ROOT
            )
    ...
    def relative_media_url(self):
        ...
        multitenant_relative_url = (
            "/".join(
                s.strip("/") for s in [settings.MEDIA_URL, multitenant_relative_url]
            )
            + "/"
        )
    ...
    def base_location(self):
        return self._value_or_setting(
            self._location, utils.parse_tenant_config_path(self.relative_media_root)
        )
    ...
    def base_url(self):
        ...
        relative_tenant_media_url = (
            "/"
            + "/".join(
                s.strip("/") for s in [self._base_url, relative_tenant_media_url]
            )
            + "/"
        )
