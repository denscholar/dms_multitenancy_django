...
class DjangoTenantsConfig(AppConfig):
    ...
    def ready(self):
        ...
        if (
            hasattr(settings, "HAS_MULTI_TYPE_TENANTS")
            and settings.HAS_MULTI_TYPE_TENANTS
        ):
            if not hasattr(settings, "TENANT_TYPES"):
                raise ImproperlyConfigured(
                    "Using multi type you must setup TENANT_TYPES setting"
                )
            if get_public_schema_name() not in settings.TENANT_TYPES:
                raise ImproperlyConfigured(
                    "get_public_schema_name() value not found as a key in TENANTS"
                )
            if not hasattr(settings, "MULTI_TYPE_DATABASE_FIELD"):
                raise ImproperlyConfigured(
                    "Using multi type you must setup MULTI_TYPE_DATABASE_FIELD setting"
                )
        else:
            ...
            if not settings.TENANT_APPS:
                raise ImproperlyConfigured(
                    "TENANT_APPS is empty. " "Maybe you don't need this app?"
                )

        if not hasattr(settings, "TENANT_MODEL"):
            raise ImproperlyConfigured("TENANT_MODEL setting not set")

        tenant_sync_router = getattr(
            settings, "TENANT_SYNC_ROUTER", "django_tenants.routers.TenantSyncRouter"
        )
        if tenant_sync_router not in settings.DATABASE_ROUTERS:
            raise ImproperlyConfigured(
                "DATABASE_ROUTERS setting must contain " "'%s'." % tenant_sync_router
            )
