...
class TenantMixin(models.Model):
    ...
    schema_name = models.CharField(
        max_length=63, unique=True, db_index=True, validators=[_check_schema_name]
    )
    ...
    def save(self, verbosity=1, *args, **kwargs):
        ...
        if has_schema and is_new and connection.schema_name != get_public_schema_name():
            ...
            has_schema
            and not is_new
            and connection.schema_name
            not in (self.schema_name, get_public_schema_name())
        ):
            raise Exception(
        ...
        if has_schema and is_new and self.auto_create_schema:
            try:
                ...
                post_schema_sync.send(
                    sender=TenantMixin, tenant=self.serializable_fields()
                )
            except Exception:
            ...
            schema_needs_to_be_sync.send(
                sender=TenantMixin, tenant=self.serializable_fields()
            )
        elif (
            not is_new
            and self.auto_create_schema
    ...
    def _drop_schema(self, force_drop=False):
        ...
        connection = connections[get_tenant_database_alias()]
        has_schema = hasattr(connection, "schema_name")
        if has_schema and connection.schema_name not in (
            self.schema_name,
            get_public_schema_name(),
        ):
            raise Exception(
        ...
        if (
            ...
            and (self.auto_drop_schema or force_drop)
        ):
            self.pre_drop()
