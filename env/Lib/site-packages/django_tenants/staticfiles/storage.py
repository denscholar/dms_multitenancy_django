...
class TenantStaticFilesStorage(TenantFileSystemStorage):
    ...
    def relative_static_root(self):
        try:
            return os.path.join(
                settings.STATIC_ROOT, settings.MULTITENANT_RELATIVE_STATIC_ROOT
            )
    ...
    def relative_static_url(self):
        ...
        try:
            if settings.REWRITE_STATIC_URLS is True:
                ...
                url = (
                    "/"
                    + "/".join(s.strip("/") for s in [url, multitenant_relative_url])
                    + "/"
                )
    ...
    def base_location(self):
        return self._value_or_setting(
            self._location, utils.parse_tenant_config_path(self.relative_static_root)
        )
