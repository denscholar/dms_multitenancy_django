...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--clone_tenant_fields", help="Clone the tenant fields.")

        parser.add_argument(
            "--db_user", help="the user for the database the default is postgres"
        )
    ...
    def handle(self, *args, **options):
        ...
        while clone_schema_from == "" or clone_schema_from is None:
            while True:
                clone_schema_from = self._input(
                    "Clone Tenant Schema ('?' to list schemas): "
                )
        ...
        while clone_tenant_fields is None or clone_tenant_fields.lower() not in [
            "no",
            "yes",
            "true",
            "false",
            ...
            while True:
                for field in self.tenant_fields:
                    if tenant_data.get(field.attname, "") == "":
                        ...
                        input_value = (
                            self._input(force_str("%s: " % input_msg)) or default
                        )
                        tenant_data[field.attname] = input_value
                tenant = self.store_tenant(
                    clone_schema_from=clone_schema_from,
    ...
    def store_tenant(self, clone_schema_from, clone_tenant_fields, **fields):
        ...
        try:
            ...
            clone_schema.clone_schema(
                clone_schema_from, tenant.schema_name, set_connection=False
            )
            return tenant
