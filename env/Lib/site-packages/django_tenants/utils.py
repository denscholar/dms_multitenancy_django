...
def schema_rename(
    ...
    sql = "ALTER SCHEMA {0} RENAME TO {1}".format(
        connection.ops.quote_name(tenant.schema_name),
    ...
    def _tenant_migration(func):
        ...
        def wrapper(*_args, **kwargs):
            try:
                ...
                raise Exception(
                    f"Decorator requires apps & schema_editor as positional arguments: {excp}"
                )

            if (
                tenant_schema
                and schema_editor.connection.schema_name != get_public_schema_name()
            ) or (
                public_schema
                and schema_editor.connection.schema_name == get_public_schema_name()
            ):
