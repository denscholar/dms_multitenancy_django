...
class MultiTypeTestCase(BaseTestCase):
    ...
    def setUpClass(cls):
        ...
        settings.MULTI_TYPE_DATABASE_FIELD = (
            "type"  # needs to be a char field length depends of the max type value
        )

        tenant_types = {
            "public": {  # this is the name of the public schema from get_public_schema_name
                "APPS": ["django_tenants", "customers"],
    ...
    def setUp(self):
        ...
        self.public_tenant = get_tenant_model()(
            schema_name=get_public_schema_name(), type="public"
        )
        ...
        self.tenant.save()
        self.domain = get_tenant_domain_model()(
            tenant=self.tenant, domain=self.tenant_domain
        )
        self.domain.save()

        self.tenant_domain2 = "tenant2.test.com"
        self.tenant2 = get_tenant_model()(schema_name="test2", type="type2")
        self.tenant2.save()
        self.domain2 = get_tenant_domain_model()(
            tenant=self.tenant2, domain=self.tenant_domain2
        )
        self.domain2.save()
    ...
    def test_public_schema_routing(self):
        ...
        """
        request_url = "/any/request/"
        request = self.factory.get("/any/request/", HTTP_HOST=self.public_domain.domain)
        self.tm.process_request(request)
