...
class BaseSyncTest(BaseTestCase):
    ...
    def setUp(self):
        ...
        with connection.cursor() as cursor:
            cursor.execute(
                "DROP SCHEMA %s CASCADE; CREATE SCHEMA %s;"
                % (
                    get_public_schema_name(),
                    get_public_schema_name(),
                )
...
class SharedAuthTest(BaseTestCase):
    def setUp(self):
        ...
        self.domain = get_tenant_domain_model()(
            tenant=self.tenant, domain="tenant.test.com"
        )
        self.domain.save()
...
class TenantTestCaseTest(BaseTestCase, TenantTestCase):
    ...
    def test_tenant_survives_after_method1(self):
        ...
        self.assertEqual(
            1,
            get_tenant_model()
            .objects.filter(schema_name=TenantTestCase.get_test_schema_name())
            .count(),
        )
...
class TenantRenameSchemaTest(BaseTestCase):
    def test_rename_schema_ok(self):
        ...
        domain.save()
        schema_rename(
            tenant=Client.objects.filter(pk=tenant.pk).first(),
...
class CloneSchemaTest(BaseTestCase):
    def test_clone_schema(self):
        ...
        clone_schema.clone_schema(
            base_schema_name="source", new_schema_name="destination"
        )

        self.assertTrue(schema_exists("source"))
        self.assertTrue(schema_exists("destination"))

    def test_clone_schema_with_existing_records_and_add_new_records_to_resulting_schema(
        self,
    ):
        """
...
class SchemaMigratedSignalTest(BaseTestCase):
    ...
    def test_signal_on_tenant_create(self):
        ...
        if executor == "simple":
            handler.assert_called_once_with(
                schema_name="test", sender=mock.ANY, signal=schema_migrated
            )
        elif executor == "multiprocessing":
